...
const CheckInPage = () => {
  ...
  const handleCapture = async (key) => {
    ...
    resizedCanvas.toBlob(async (blob) => {
      ...
      formData.append("upload_preset", "Warehouse"); // Replace with your Cloudinary upload preset
      ...
      try {
        const response = await axios.post(
          `https://api.cloudinary.com/v1_1/dodxop7lz/image/upload`, // Replace with your Cloudinary cloud name
          formData
        );
  ...
  const handleCheckIn = async () => {
    ...
    const status =
      checkInHour > 9 || (checkInHour === 9 && checkInMinute > 15)
        ? "Late"
        : "Success";
  ...
  const handleCheckOut = async () => {
    ...
    const status =
      checkOutHour > 19 || (checkOutHour === 19 && checkOutMinute >= 0)
        ? "Overtime"
        : "Success";
